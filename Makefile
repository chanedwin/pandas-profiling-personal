...
install-spark:
	SPARK_VERSION=2.4.7 HADOOP_VERSION=2.7 SPARK_HOME=/tmp/spark-2.4.7-bin-hadoop2.7 \
    curl http://apache.mirror.gtcomm.net/spark/spark-$SPARK_VERSION/spark-$SPARK_VERSION-bin-hadoop$HADOOP_VERSION.tgz \
    --output /tmp/spark-$SPARK_VERSION-bin-hadoop$HADOOP_VERSION.tgz cd /tmp && \
